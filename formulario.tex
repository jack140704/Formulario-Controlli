...
\vspace{0.5mm} % <-- Spazio ulteriormente ridotto
...
Simulare con \texttt{step(T/(Gs*Gf))} per le specifiche in transitorio. Se $\hat{s}$ non rispettato aumento provo ad aumentare $m_{i}$, se $t_{s,\alpha\%}$ non rispettato aumento provo ad aumentare $\omega_{des}$. (Le altre su inseguimento e disturbi sono verificate se $K_c$ e $\nu$ sono OK).
...
& \mbox{$K_c = K_{c,in} 10^{\frac{-|K_{c,dB}^{sub}|}{20}}$}
