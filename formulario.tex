...
\renewcommand{\arraystretch}{1.2}
\begin{tabularx}{\linewidth}{|>{\centering\arraybackslash}X|>{\centering\arraybackslash}p{3.8cm}|}
\hline
...
$\mathcal{L}\{\varepsilon(t)\} = \frac{1}{s} \quad \mathcal{Z}\{\varepsilon(t)\} = \frac{z}{z-1}$ \\
\hline
\end{tabularx}
\end{multicols}
}}
...
\parbox[t]{0.45\linewidth}{$Re(\lambda_i) \leq 0 \; \forall i$\\$Re(\lambda_i) = 0, \mu_g = 1$} & 
\parbox[t]{0.45\linewidth}{$\forall x_0 \in \mathbb{R}^n,$\\$x_{zi}(k)$ limitata\\$\lim_{k\to\infty} x_{zi}(k) \neq 0$} \hfill
\parbox[t]{0.45\linewidth}{$|\lambda_i| \leq 1 \; \forall i$\\$|\lambda_i| = 1, \mu_g = 1$} \\
\hline
BIBO stabile & 
