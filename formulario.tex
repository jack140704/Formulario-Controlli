...
\end{tabularx}
\vspace{2pt}
\textbf{Nota}: per creare sistema continuo: \texttt{sys = ss(A, B, C, D)} \\ discreto: \texttt{sys = ss(A, B, C, D, -1)}\\
per simulare risposta libera: \texttt{initial(sys, x0)}\\
per simulare risposta forzata: \texttt{step(sys)} o \texttt{lsim(sys, u, t)}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.48\textwidth}
\vspace{0pt}
...
\vspace{4pt}
\textbf{Nota:} Molteplicità geometrica \quad $m_g(\lambda_0) = n - \text{rank}(A - \lambda_0 I_n)$
}}
\end{center}
...
\sub{b) Calcolo $\nu$ e $K_c$ da $\varepsilon_r$ (\texttt{Tabella Part7 pag.44})} 
